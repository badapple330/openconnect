...
<s:if test="%{#session.userFlg >= 1 && #session.userId != null}">
    ...
    <h1>決裁手続き一覧</h1>
<br>
    <p>ログイン中ユーザーID:<s:property value="#session.userId" /></p>


    <s:form action="DecisionDetailSelectAction">
        プロジェクト検索<span class="pg_none"><br></span>
    ...
    <p>現在進行中の決裁：全<s:property value="%{decisionDetailList1.size() + decisionDetailList2.size()}" />チーム</p>
					...
					<s:else>
						<s:form action="DecisionPreviewAction">
	                            	...
	                            	<input type="hidden" name="type" value="1">
	                                    <button type="submit" >Pre　<br>view</button>
                       			 </s:form>
					</s:else>
				</s:if>
                           			...
                           			<input type="hidden" name="jDrafterId" value="<s:property value="jDrafterId" />">
                           				<input type="hidden" name="kDrafterId" value="<s:property value="kDrafterId" />">
                           					...
                           					<s:if test="decisionStatus != 12">
                                   				<button type="submit"  onclick='return confirm("遡求は一度編集を始めると取り消しができません。\n必ず先生の承認をもらい完了させてください。\nよろしいですか？");'>編集</button>
                                  			</s:if>
                                  			<s:else>
                                  				<button type="submit" >編集</button>
                                  			</s:else>
	                	 ...
	                	 <s:else>
				                     	...
				                     	<s:if test="kPermiterId3 > 0">
											<input type="hidden" name="jDrafterId" value="<s:property value="jDrafterId" />">
												...
												<s:if test="decisionStatus != 11">
													<button type="submit"  onclick='return confirm("変更は一度編集を始めると取り消しができません。\n必ず先生の承認をもらい完了させてください。\nよろしいですか？");'>実施<br>編集</button>
												</s:if>
												<s:else><!-- 2回目以降 -->
													<button type="submit" >実施<br>編集</button>
				                ...
				                <!-- 契約の変更 -->
				                     	<s:else>
											<input type="hidden" name="kDrafterId" value="<s:property value="kDrafterId" />">
												<input type="hidden" name="type" value="5">
													<button type="submit" >契約<br>編集</button>
											</s:else>
									</s:if>
								<!-- 実施の変更(差し戻し・却下時) -->
								...
								<!-- 実施兼契約の変更 -->
				                     <s:else>
											<input type="hidden" name="kDrafterId" value="<s:property value="kDrafterId" />">
												...
												<s:else><!-- 2回目以降 -->
													<button type="submit" >実施兼契約<br>編集</button>
												</s:else>
											</s:else>
								</s:form>
						</s:else>
                ...
                <s:form action="DecisionDetailApplicationAction">
						...
						<!-- 実施(差し戻し/却下時含む)の申請-->
							<s:if test="%{decisionType == '実施' }">
									<input type="hidden" name="stringId" value="<s:property value="jImpId" />">
											<button type="submit"  onclick='return confirm("よろしいですか？");'>実施<br>申請</button>
													</s:if>
						<!-- 契約(差し戻し/却下時含む)の申請 -->
							<s:elseif test="%{decisionType == '契約'}">
									<input type="hidden" name="stringId" value="<s:property value="kImpId" />">
											<button type="submit"  onclick='return confirm("よろしいですか？");'>契約<br>申請</button>
													</s:elseif>
						<!-- 実施兼契約(差し戻し/却下時含む)の申請 -->
							<s:else>
									<input type="hidden" name="stringId" value="<s:property value="jkImpId" />">
											<button type="submit"  onclick='return confirm("よろしいですか？");'>実施兼契約<br>申請</button>
													</s:else>
				</s:form>
                	</s:if>
					...
					<s:else>
						...
						<s:form action="DecisionDetailCancelAction">
	                   			...
	                   			<s:else>
	                   				<button type="submit"  onclick='return confirm("よろしいですか？");'>申請<br>取り下げ</button>
	                   			</s:else>
						</s:form>
							</s:if>
					...
					<s:if test="recourseStatus == 1">
		                	...
		                	<s:if test="decisionStatus != 6">
			                	<s:form action="DecisionDetailRecourseAction">
		                    		...
		                    		<!-- 初回時 -->
		                    		<s:if test="decisionStatus == 12">
		                    				<button type="submit"  onclick='return confirm("遡求申請は申請の取り消しができません。\nよろしいですか？");'>遡求<br>申請</button>
		                    						</s:if>
		                    		<!-- 2回目以降・差し戻し時 -->
		                    		<s:elseif test="decisionStatus == 10">
		                    				<button type="submit" onclick='return confirm("よろしいですか？");'>遡求<br>申請</button>
		                    						</s:elseif>
		                    		<!-- 遡求承認完了後の変更申請可時 -->
		                    		<s:else></s:else>
				                </s:form>
					...
					<s:form action="DecisionDetailChangeAction">
		                ...
		                <s:if test="kPermiterId3 > 0 && decisionStatus == 11">
		                        <input type="hidden" name="decisionType" value="実施">
	                                	<button type="submit" onclick='return confirm("変更申請は申請の取り消しができません。\nよろしいですか？");'>実施変更<br>申請</button>
                        						</s:if>
                        <!-- 遡求・変更ともに完了時 -->
		                ...
		                <s:else>
		                        <input type="hidden" name="decisionType" value="契約">
	                                	<button type="submit" onclick='return confirm("よろしいですか？");'>契約変更<br>申請</button>
                        						</s:else>
					 </s:form>
                        </s:if>
		             ...
		             <s:form action="DecisionDetailChangeAction">
		                        <input type="hidden" name="decisionId" value="<s:property value="decisionId" />">
		                        	<input type="hidden" name="userId" value="<s:property value="#session.userId" />">
		                        		<input type="hidden" name="permitStatus" value="<s:property value="permitStatus" />">
		                					<input type="hidden" name="decisionType" value="実施">
			                                	<button type="submit" onclick='return confirm("よろしいですか？");'>実施変更<br>申請</button>
			         </s:form>
		                </s:elseif>
		                <!-- 実施兼契約の変更 -->
                        <s:else>
		                        	...
		                        	<input type="hidden" name="userId" value="<s:property value="#session.userId" />">
		                        		<input type="hidden" name="permitStatus" value="<s:property value="permitStatus" />">
		                        			<input type="hidden" name="decisionType" value="実施兼契約">
		                                		<button type="submit" onclick='return confirm("変更申請は申請の取り消しができません。\nよろしいですか？");'>実施兼契約<br>変更<br>申請</button>
	                 </s:form>
                        </s:else>
				</s:if>
				<!-- 承認完了済み(遡求なし・変更なし時) -->
	        ...
	        <tr>
	        	<td colspan="2">
	        		<s:form action="DecisionPreviewAction">
		                        <input type="hidden" name="decisionId" value="<s:property value="decisionId" />">
		                        	<input type="hidden" name="type" value="7">
		                                <button type="submit" >遡求Preview</button>
                    </s:form>
	        	</td><td colspan="3">―</td>
	        </tr>
        </s:if>
    ...
    <!-- プレビューボタン -->
        ...
        <!-- 差し戻しボタン -->
            ...
            <!-- 先生のみ -->
                ...
                <!-- 先生が差し戻し可能な時 -->
                    <s:if test="permitStatus == 2">
                            <button type="submit" onclick='return confirm("よろしいですか？");'>差し戻し</button>
                    </s:if>
                    <s:else>―</s:else>
                </s:if>
        ...
        <!-- 却下ボタン(遡求を除く) -->
            ...
            <!-- 先生のみ -->
                ...
                <!-- 先生が却下可能な時 -->
                    <s:if test="permitStatus == 2">
                            <button type="submit" onclick='return confirm("よろしいですか？");'>却下</button>
                    </s:if>
                    <s:else>―</s:else>
                </s:if>
                        ...
                        <s:if test="permitStatus == 2">
                            <s:form action="DecisionDetailPermitAction">
                                ...
                                <input type="hidden" name="permitStatus" value="2">
                                        <button type="submit" onclick='return confirm("よろしいですか？");'>実施<br>承認<br>3人目</button>
                            </s:form>
                        </s:if>
                        <s:elseif test="permitStatus == 1">
                            リーダー<br>2人目対応中
                        ...
                        <s:if test="permitStatus == 0">
                            <s:form action="DecisionDetailPermitAction">
                                ...
                                <input type="hidden" name="permitStatus" value="0">
                                        <button type="submit" onclick='return confirm("よろしいですか？");'>実施<br>承認<br>1人目</button>
                            </s:form>
                        </s:if>
                    <!-- リーダー2の承認 -->
                        <s:elseif test="permitStatus == 1">
                            <s:form action="DecisionDetailPermitAction">
                                ...
                                <input type="hidden" name="permitStatus" value="1">
		                                ...
		                                <s:else>
		                                        <button type="submit" onclick='return confirm("よろしいですか？");'>実施<br>承認<br>2人目</button>
		                                </s:else>
                            </s:form>
                        </s:elseif>
                        ...
                        <s:if test="permitStatus == 2">
                            <s:form action="DecisionDetailPermitAction">
                                ...
                                <input type="hidden" name="permitStatus" value="2">
		                                <s:if test="%{decisionType == '契約'}">
		                                        <s:if test="decisionStatus == 6">
		                                        	<button type="submit" onclick='return confirm("よろしいですか？");'>遡求<br>承認</button>
		                                        </s:if>
		                                        <s:else>
		                                        	<button type="submit" onclick='return confirm("よろしいですか？");'>契約<br>承認<br>3人目</button>
		                                        </s:else>
		                                </s:if>
		                                <s:elseif test="%{decisionType == '実施兼契約'}">
		                                        ...
		                                        <s:else>
		                                        	<button type="submit" onclick='return confirm("よろしいですか？");'>実施兼契約<br>承認<br>3人目</button>
		                                        </s:else>
		                                </s:elseif>
                            </s:form>
                        </s:if>
                    ...
                    <!-- リーダー1の承認 -->
                        <s:if test="permitStatus == 0">
                            <s:form action="DecisionDetailPermitAction">
                                ...
                                <input type="hidden" name="permitStatus" value="0">
		                                <s:if test="%{decisionType == '契約'}">
		                                        <button type="submit" onclick='return confirm("よろしいですか？");'>契約<br>承認<br>1人目</button>
		                                </s:if>
		                                <s:elseif test="%{decisionType == '実施兼契約'}">
		                                        <button type="submit" onclick='return confirm("よろしいですか？");'>実施兼契約<br>承認<br>1人目</button>
		                                </s:elseif>
                            </s:form>
                        </s:if>
                    <!-- リーダー2の承認 -->
                        <s:elseif test="permitStatus == 1">
                            <s:form action="DecisionDetailPermitAction">
                                ...
                                <input type="hidden" name="permitStatus" value="1">
		                                ...
		                                <s:else>
		                                    <s:if test="%{decisionType == '契約'}">
		                                        <button type="submit" onclick='return confirm("よろしいですか？");'>契約<br>承認<br>2人目</button>
		                                    </s:if>
		                                    <s:elseif test="%{decisionType == '実施兼契約'}">
		                                        <button type="submit" onclick='return confirm("よろしいですか？");'>実施兼契約<br>承認<br>2人目</button>
		                                    </s:elseif>
		                                </s:else>
                            </s:form>
                        </s:elseif>
	        	...
	        	<td colspan="3">
	        		<s:if test="%{#session.userFlg == 3 && decisionStatus == 5 && time== 1}">
	        				<s:form action="DecisionDetailCompleteAction">
			                        <input type="hidden" name="decisionId" value="<s:property value="decisionId" />">
			                        	<button type="submit" >リリース完了</button>
		                    </s:form>
	        		</s:if>
	        		<s:else></s:else>
	        	</td>
...
<s:else>
	...
	<s:form action="GetAddressAction">
		<input type="submit" class="return_btn" value="戻る">
	</s:form>
