...
$(function(){
	  //テキストリンクをクリックしたら
	    ...
	    modalResize();
	    //モーダルウィンドウを表示
	        $("#modal-bg,#editMain").fadeIn("slow");

	        var index = $('.editOpen').index($(this));
			...
			$('#true-edit').val($('.decision_detail_id').eq(index).text());

	        $(".modal-close").click(function(){
	            $("#editMain,#modal-bg").fadeOut("slow",function(){
	    ...
	    //画面の左上からmodal-mainの横幅・高さを引き、その値を2で割ると画面中央の位置が計算できます
	        ...
	        function modalResize(){
	            var w = $(window).width();
	            var h = $(window).height();
	            var cw = $("#editMain").outerWidth();
	            var ch = $("#editMain").outerHeight();
	            //取得した値をcssに追加する
	                $("#editMain").css({
	                    "left": ((w - cw)/2) + "px",
	                    "top": ((h - ch)/2) + "px"
