	...
	<s:if test="%{#session.userFlg >= 1 && #session.userId != null}">
		...
		<div class="col-sm-12" id="topWord">
			...
			<s:form action="DecisionDetailSelectAction">
				<label style="position: relative; z-index: -1;">プロジェクト検索 </label>
				<span class="pg_none"><br></span>
			...
			<s:if test="%{#session.userFlg == 2}">
						...
						<s:iterator value="decisionBeginList">
							...
							<s:if test="%{decisionBeginList.size() > 0}">
								<div class="pad">
									...
									<p>
										自チームの未作成決裁：
										<s:property value="%{decisionBeginList.size()}" />
										個
									</p>
									...
									<s:form action="DecisionDetailInsertAction">
										...
										<table class="table table-responsive" border="1">
											...
											<tr>
												<td>実施兼契約決裁で行う場合のみチェック <input type="checkbox"
													name="decisionType" value="実施兼契約">
												</td>
												<td><input type="hidden" name="projectId"
													value="<s:property value="projectId" />"> <input
													type="hidden" name="userId"
													value="<s:property value="#session.userId" />"> <input
													class="btn btn-default btn-lg center-block" type="submit"
													value="追加"></td>
											</tr>
										</table>
				...
				<s:iterator value="decisionDetailList2">
					<div class="row">
						<div class="col-sm-7">
							...
							<s:if test="%{decisionDetailList2.size() > 0}">
								...
								<table class="table table-responsive" border="1">
									<tr>
										<!-- プロジェクト名 -->
										<th colspan="2" rowspan="2"><s:property
												value="projectName" /></th>
										<!-- 決裁種類 -->
										<th colspan="3"><s:property value="decisionType" /></th>
									</tr>
									<tr>
										<!-- 決裁状況 -->
										<td colspan="3"><s:if test="decisionStatus == 0">
	                    作成中
	                </s:if> <s:elseif test="decisionStatus == 1">
	                    差し戻し中
	                </s:elseif> <s:elseif test="decisionStatus == 2">
	                    却下中
	                </s:elseif> <s:elseif test="decisionStatus == 3">
	                    申請中
	                </s:elseif> <s:elseif test="decisionStatus == 4">
	                    [変更]申請中
	                </s:elseif> <s:elseif test="decisionStatus == 5">
	                    すべて承認済み
	                </s:elseif> <s:elseif test="decisionStatus == 6">
	                    [遡求]申請中
	                </s:elseif> <s:elseif test="decisionStatus == 8">
	                    [変更]差し戻し中
	                </s:elseif> <s:elseif test="decisionStatus == 9">
	                    [変更]却下中
	                </s:elseif> <s:elseif test="decisionStatus == 10">
	                    [遡求]差し戻し中
	                </s:elseif> <s:elseif test="decisionStatus == 11">
	                    [変更]編集中
	                </s:elseif> <s:elseif test="decisionStatus == 12">
	                    [遡求]編集中
	                </s:elseif></td>
									</tr>
									<tr>
										<s:if test="%{decisionType == '実施' || decisionType == '契約'}">
											<th><s:if test="changeStatus == 1">[変更]<span
														class="pg_none"><br></span>実施</s:if> <s:else>実施</s:else></th>
											<th><s:if test="changeStatus == 1">[変更]<span
														class="pg_none"><br></span>契約</s:if> <s:else>契約</s:else></th>
										</s:if>
										<s:else>
											<th colspan="2"><s:if test="changeStatus == 1">[変更]<span
														class="pg_none"><br></span>実施兼契約</s:if> <s:else>実施兼契約</s:else>
											</th>
										</s:else>

										<th class="title">
											<!-- 変更・遡求(最終承認済み後に表示) --> <s:if test="decisionStatus >= 4">
												<!-- 終了日を過ぎた場合 -->
												<s:if test="time == 1">遡求</s:if>
												<!-- 最終承認から終了日まで -->
												<s:else>変更</s:else>
											</s:if> <!-- 未-最終承認時 --> <s:else>―</s:else>
										</th>
									...
									<tr>
										...
										<s:if test="%{decisionType == '実施' || decisionType == '契約'}">
											<td><s:if test="changeStatus != 1">
													...
													<!-- [変更]実施のプレビュー -->
													<s:if
														test="%{decisionStatus != 8 && decisionStatus != 9 && jApplyDay != null}">
														<s:form action="DecisionPreviewAction">
															<input type="hidden" name="decisionId"
																value="<s:property value="decisionId" />">
											...
											<td><s:if test="%{decisionType == '契約'}">
													...
													<s:else>
														<!-- [変更]契約のプレビュー -->
														<s:if
															test="decisionStatus != 8 && decisionStatus != 9 && decisionStatus != 11">
															<s:form action="DecisionPreviewAction">
																<input type="hidden" name="decisionId"
																	value="<s:property value="decisionId" />">
												...
												</s:if> <!-- 契約未着手時 --> <s:else>―</s:else></td>
										</s:if>
										<s:else>
											<td colspan="2"><s:if test="changeStatus != 1">
													<!-- 実施兼契約の編集 -->
													<s:if test="decisionStatus < 3">
														<s:form action="DecisionSelectAction">
															<input type="hidden" name="kDrafterId"
										...
										<!-- 変更・遡求編集ボタン -->
										<td>
											<!-- 最終承認済み後・差し戻し・却下時に表示 --> <s:if
												test="decisionStatus == 5 || decisionStatus >= 8">
												...
												<s:else>
													<!-- 遡求編集(申請後2回目以降、差し戻し時) -->
													<s:if
														test="recourseStatus == 1 && (decisionStatus == 10 || decisionStatus == 12)">
														<s:form action="DecisionSelectAction">
															<input type="hidden" name="decisionId"
																value="<s:property value="decisionId" />">
															<input type="hidden" name="jDrafterId"
										...
										<!-- 承認状況 -->
										<td>
											<!-- 申請中(変更・遡求申請含む)の時 --> <s:if
												test="decisionStatus == 3 || decisionStatus == 4 || decisionStatus == 6">
												<!-- 実施の時 -->
					                    ...
					                    1人目<br>承認待ち
											...
											</s:if> <!-- 作成中の時 --> <s:elseif test="decisionStatus < 3">
	                   ...
	                   [遡求]<br>未申請
	                </s:elseif> <!-- 承認済みの時 --> <s:elseif test="decisionStatus == 5">
												<s:if test="time != 1">
	                    	...
	                    	[遡求]可
										...
										<!-- 申請ボタン -->
										<td>
											<!-- 未承認が残っている間 --> <s:if test="decisionStatus < 5">
												...
												<s:if test="decisionStatus < 3">
													<s:form action="DecisionDetailApplicationAction">
														...
														<input type="hidden" name="decisionId"
															value="<s:property value="decisionId" />">
														<input type="hidden" name="decisionType"
															value="<s:property value="decisionType" />">
														<input type="hidden" name="userId"
															value="<s:property value="#session.userId" />">
														<s:token />
												...
												<s:else>
													...
													<s:if test="permitStatus == 0">
														<s:form action="DecisionDetailCancelAction">
															...
															<input type="hidden" name="decisionId"
																value="<s:property value="decisionId" />">
															<input type="hidden" name="decisionStatus"
																value="<s:property value="decisionStatus" />">
															<input type="hidden" name="decisionType"
																value="<s:property value="decisionType" />">
															<input type="hidden" name="userId"
																value="<s:property value="#session.userId" />">
													...
													<s:elseif test="%{decisionType == '実施'}">
														<s:form action="DecisionDetailChangeAction">
															...
															<button class="btn btn-default" type="submit"
																onclick='return confirm("よろしいですか？");'>実施変更申請</button>
														</s:form>
													</s:elseif>
													<!-- 実施兼契約の変更 -->
													<s:else>
									...
									<s:if test="%{sApplyDay != null && decisionStatus != 10}">
										<tr>
											<td colspan="2"><s:form action="DecisionPreviewAction">
													<input type="hidden" name="decisionId"
														value="<s:property value="decisionId" />">
													<input type="hidden" name="type" value="7">
				...
				<div class="row">
					<div class="col-sm-7"
						...
						<table class="table table-responsive" border="1">
							<tr>
								<!-- プロジェクト名 -->
								<th colspan="2" rowspan="2"><s:property value="projectName" />
								</th>
								<!-- 決裁種類 -->
								<th colspan="3"><s:property value="decisionType" /></th>
							</tr>
							...
							<tr>
								...
								<!-- 一般の表示 -->
								<s:else>
									<th colspan="3" rowspan="2" class="title"></th>
								</s:else>
							</tr>
							<tr>
								<!-- プレビューボタン -->
								<s:if test="%{decisionType == '実施' || decisionType == '契約'}">
									<td>
										<!-- 実施のプレビュー --> <s:if test="changeStatus != 1">
											<s:form action="DecisionPreviewAction">
												<input type="hidden" name="decisionId"
													value="<s:property value="decisionId" />">
												<input type="hidden" name="type" value="1">
									...
									<td>
										<!-- 契約のプレビュー --> <s:if test="%{decisionType == '契約'}">
											<s:if test="changeStatus != 1">
												<s:form action="DecisionPreviewAction">
													<input type="hidden" name="decisionId"
														value="<s:property value="decisionId" />">
								...
								<s:else>
									<td colspan="2">
										<!-- 実施兼契約のプレビュー --> <s:if test="changeStatus != 1">
											<s:form action="DecisionPreviewAction">
												<input type="hidden" name="decisionId"
													value="<s:property value="decisionId" />">
												<input type="hidden" name="type" value="3">
								...
								<s:if test="%{#session.userFlg >= 2}">
									...
									<td>
										<!-- 申請中の時 --> <s:if
											test="decisionStatus == 3 || decisionStatus == 4">
